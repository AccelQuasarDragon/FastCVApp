...
def FCVA_update_resources(*args, sourcelocationVAR = False):
	...
	if hasattr(sys, "_MEIPASS"):
		...
		if sourcelocationVAR:
			...
			min = min(len(newtempsourcesplit), len(sourcelocationVARsplit))
			commonpaths = []
			for intcheck in range(min):
				print("checking sequentially from last", min, intcheck, newtempsourcesplit[-intcheck], sourcelocationVARsplit[-intcheck])
				if newtempsourcesplit[-intcheck] == sourcelocationVARsplit[-intcheck]:
					commonpaths.append(newtempsourcesplit[-intcheck])
