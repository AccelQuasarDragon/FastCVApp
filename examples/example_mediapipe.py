try:
    ...
    def draw_landmarks_on_image(annotated_image, detection_result):
        try:
            ...
            for idx in range(len(pose_landmarks_list)):
                ...
                #pose landmarks are already normalized, just draw smth temporary for now:
                ...
                print(len(pose_landmarks_list),  "LLANDM", flush=True)
                annotated_image = cv2.circle(annotated_image, (
                    int(annotated_image.shape[1]*pose_landmarks.x), 
                    int(annotated_image.shape[0]*pose_landmarks.y)), 3, (0,0,255), -1)
    ...
    import os
    print("example_mediapipe died!", e, "what is __name__?", __name__, os.getpid())
    import traceback
    print("full exception", "".join(traceback.format_exception(*sys.exc_info())))
    import time
    time.sleep(300)