...
def cv_basic_haar(retVAR, frameVAR):
    if retVAR:
        #they resized it to be less laggy:
        w_size = (700,500)
        frame_var = cv2.resize(frameVAR,w_size)
...
'''
        ...
        def blit_from_shared_memory(self, *args):
            ...
            import dill
            dilling = dill.pickles(cv_haar_cascade_async)
            if dilling:
                self.what = FCVApool.apply_async(cv_haar_cascade_async, args=(ret, frame, shared_analysis_dict, self.frame_int)) 
                # self.what = FCVApool.apply_async(parallelize_cv_func, args=(cv_basic_haar, ret, frame, shared_analysis_dict, self.frame_int)) 
            else:
