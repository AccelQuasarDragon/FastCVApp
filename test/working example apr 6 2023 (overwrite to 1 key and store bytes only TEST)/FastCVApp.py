...
'''
        ...
        def blit_from_shared_memory(self, *args):
            ...
            if len(shared_analysis_dict) > 0:
                ...
                frame = np.frombuffer(shared_analysis_dict[max_key], np.uint8).copy().reshape(1080, 1920, 3)
    ...
    MainApp.shared_timer_dictVAR = args[3]
    MainApp().run()
    ...
    try:
        ...
        while True:
            ...
            if "mp_ready" in shared_metadata_dict.keys() and [shared_metadata_dict[key] for key in shared_metadata_dict.keys() if key == "toggleCV"] == [True]:
                ...
                spf = 1/30
                frame_number = int((time.time() - shared_timer_dictVAR["start_time"] )/spf) #need this to round down i think
                if (time_elapsed > 1./frame_rate) and frame_number >= 0:
                    # time_og = time.time()
                    # reference: https://stackoverflow.com/a/47867180
            ...
            #         # read the latest frame here and stuff it in the shared memory for open_appliedcv to manipulate
                    ...
                    if cap.more(): #for FileVideoStream
                        ...
                        shared_metadata_dict["latest_cap_frame"] = frame.tobytes() #THIS LINE IS THE BOTtLENECK, I FOUND YOU
            ...
            #         # print("cv2 .read() takes long???", time_2 - time_og, 1./frame_rate, flush= True)
                ...
                if (time_2 - time_og) > 0:
                    if (1/(time_2 - time_og))<100:
                        ...
                        print("cv2 .read() takes long???", "fps:", 1/(time_2 - time_og) , drift , frame_number , index_count, flush= True)
                # else:
    ...
    try:
        ...
        while True:
            ...
            if "kivy_run_state" and "latest_cap_frame" in shared_metadata_dict.keys() and [shared_metadata_dict[key] for key in shared_metadata_dict.keys() if key == "toggleCV"] == [True]:
                ...
                #readbytes: 
                actualframe = np.frombuffer(shared_metadata_dict["latest_cap_frame"], np.uint8).copy().reshape(1080, 1920, 3)
                shared_analysis_dict[1] = appliedcv(actualframe,shared_analysis_dict ,shared_metadata_dict)
...
class FCVA():
    ...
    def run(self):
        if __name__ == "FastCVApp":
            ...
            #start time dict:
            shared_timer_dict = shared_mem_manager.dict()
            ...
            read_subprocess = FCVA_mp.Process(target=open_media, args=(shared_metadata_dict, fps, self.source, shared_timer_dict))
